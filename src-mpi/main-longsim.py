...
ggap = 3.*1/3.*1/6.*5.1*0.385*1e-4#0.5*0.00017e-1
ggaphub = 0.3*1/3.*1/6.*5.1*0.385*1e-4#1.0*0.00017e-1
gjtau = 160.0
dthres = 17.5  # spatial cutoff distance to def GJ connection
isletsize = 40  # islet size of interest (None for whole islet)
hetVar = 0.005
tstop = 60e3#350e3  # usually in [ms]
...
#hubsList = [1649]
hubsList = [471,1204,114,1531,603,336,851,497,266,1649,1352,1202,747,579,1605,1420,806]
...
for i in range(ncells):
    if i not in hubsList:
        ...
        if isImitateExp:
            if mode==1 and i==579:#imagedHubs[1]):
